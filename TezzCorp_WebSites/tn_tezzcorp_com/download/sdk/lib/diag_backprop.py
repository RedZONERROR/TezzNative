...
Prints: logits, softmax probs, losses, dlogits, dW2 at step 0
        and loss each epoch — if loss decreases, backprop is correct.
"""
import ctypes, os, struct, sys

# Load tzcpu.dll
dll_path = os.path.join(os.path.dirname(__file__), "run", "tzcpu.dll")
...
# Diverse inputs: class k → pixels[k*bw..(k+1)*bw]=0.9, rest=0.1
bw = IN // C
for i in range(B):
    row = [0.1] * IN
    cls = i % C
...
for step in range(30):
    ...
    status = "⬇ GOOD" if step > 0 else "init"
