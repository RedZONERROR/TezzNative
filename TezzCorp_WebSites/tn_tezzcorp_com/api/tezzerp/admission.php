...
/**
 * Resolve a safe default value for NOT NULL fields with no default.
 */
function admissionRequiredDefaultByType(array $columnMeta): mixed
{
    ...
    if (preg_match('/^enum\((.*)\)$/', $type, $matches)) {
        ...
        $values = array_values(array_filter(array_map(static fn($v): string => trim((string)$v), $values), static fn($v): bool => $v !== ''));
        foreach (['inactive', 'pending', 'new', 'draft', 'disabled', '0', 'false', 'no', 'active'] as $preferred) {
    ...
    if (preg_match('/^(tinyint|smallint|mediumint|int|bigint|decimal|float|double|real|numeric|bit|bool|boolean)/', $type)) {
        return 0;
    }
...
/**
 * Fill required (NOT NULL + no default) columns that are missing from payload.
 *
 * @param array<string, mixed> $rowData
 * @param array<string, array<string, mixed>> $tableMeta
 * @param string[] $skip
 * @return array<string, mixed>
 */
function admissionFillRequiredDefaults(array $rowData, array $tableMeta, array $skip = []): array
{
    $skipLookup = [];
    foreach ($skip as $column) {
        $skipLookup[strtolower(trim((string)$column))] = true;
    }

    foreach ($tableMeta as $lowerName => $meta) {
        $field = trim((string)($meta['Field'] ?? ''));
        if ($field === '') {
            continue;
        }
        if (isset($skipLookup[$lowerName])) {
            continue;
        }
        ...
        $rowData[$field] = admissionRequiredDefaultByType($meta);
    }
...
try {
    ...
    $studentIdColumn = apiFirstColumn($studentColumns, ['id', 'student_id']);
    $studentUidColumn = apiFirstColumn($studentColumns, ['uid', 'student_uid']);
    $studentStatusColumn = apiFirstColumn($studentColumns, ['status', 'student_status', 'is_active', 'active']);

    if ($studentUidColumn === null) {
    ...
    if ($action === 'update') {
        ...
        if ($targetUid !== '' && $studentUidColumn !== null) {
            $existingStmt = $pdo->prepare(
                'SELECT '
                . ($studentIdColumn !== null ? apiIdent($studentIdColumn) : 'NULL')
                . ' AS student_id, '
                . apiIdent($studentUidColumn)
                . ' AS uid FROM students WHERE '
                . apiIdent($studentUidColumn)
                . ' = :uid LIMIT 1'
            );
            $existingStmt->execute([':uid' => $targetUid]);
        } elseif ($targetStudentId > 0 && $studentIdColumn !== null) {
            $existingStmt = $pdo->prepare(
                'SELECT ' . apiIdent($studentIdColumn) . ' AS student_id, '
                . apiIdent($studentUidColumn) . ' AS uid
                 FROM students
                 WHERE ' . apiIdent($studentIdColumn) . ' = :id
                 LIMIT 1'
            );
            $existingStmt->execute([':id' => $targetStudentId]);
        } elseif ($targetUid !== '' && $studentIdColumn !== null && ctype_digit($targetUid)) {
            $existingStmt = $pdo->prepare(
                'SELECT ' . apiIdent($studentIdColumn) . ' AS student_id, '
                . apiIdent($studentUidColumn) . ' AS uid
                 FROM students
            ...
            $existingStmt->execute([':id' => (int)$targetUid]);
        } else {
            jsonResponse(400, [
                'success' => false,
                'message' => 'Student lookup column is not available for update'
            ]);
        }
    ...
    $documentMeta = admissionTableMeta($pdo, 'documents');
    ...
    if ($action === 'create') {
        ...
        if ($studentStatusColumn !== null) {
            // Old ERP parity: fresh admissions remain non-active until explicit approval.
            $statusMeta = $studentMeta[strtolower($studentStatusColumn)] ?? [];
            $studentData[$studentStatusColumn] = admissionRequiredDefaultByType($statusMeta ?: ['Type' => 'varchar(32)']);
        }
        if (isset($studentColumnMap['admission_date']) && empty($studentData['admission_date'])) {
            $studentData['admission_date'] = date('Y-m-d');
        }
        $studentData = admissionFillRequiredDefaults($studentData, $studentMeta, [$studentIdColumn ?? '', $studentUidColumn]);

        $studentFields = array_keys($studentData);
        $studentFields[] = $studentUidColumn;
        ...
        if (!empty($documents) && isset($documentColumnMap['uid'])) {
            ...
            $documents = admissionFillRequiredDefaults($documents, $documentMeta, ['id', 'uid']);
        ...
        $studentId = (int) ($existingStudent['student_id'] ?? 0);
        if ($uid === '' || ($studentIdColumn !== null && $studentId <= 0)) {
        ...
        if (!empty($studentData)) {
            ...
            if (!empty($set)) {
                $whereSql = ($studentIdColumn !== null)
                    ? (' WHERE ' . apiIdent($studentIdColumn) . ' = :id')
                    : (' WHERE ' . apiIdent($studentUidColumn) . ' = :uid');
                if ($studentIdColumn === null) {
                    unset($params[':id']);
                    $params[':uid'] = $uid;
                }
                $updateSql = 'UPDATE students SET ' . implode(', ', $set) . $whereSql;
    ...
    // Gold-plan chargeable messaging trigger (configured per school from EDU).
    if ($action === 'create') {
        try {
            $messagePayload = [
                'student_uid' => (string)$uid,
                'student_name' => (string)($studentData['full_name'] ?? $payload['full_name'] ?? ''),
                'father_name' => (string)($studentData['father_name'] ?? $payload['father_name'] ?? ''),
                'class_name' => (string)($studentData['applying_class'] ?? $studentData['class'] ?? $payload['applying_class'] ?? $payload['class'] ?? ''),
                'admission_date' => (string)($studentData['admission_date'] ?? $payload['admission_date'] ?? date('Y-m-d')),
                'mobile_number' => (string)($studentData['mobile_number'] ?? $payload['mobile_number'] ?? ''),
            ];
            $recipients = [];
            ...
            erpMessageTrigger(
                ...
                $recipients,
                [
                    'reference_type' => 'student_admission',
                    'reference_id' => (string)$uid,
                    'actor_id' => $userId,
                ]
            );
        } catch (Throwable $ignore) {
            // Messaging failures must not block admission.
        }
    }
